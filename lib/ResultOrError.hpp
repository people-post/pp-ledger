...
struct RoeErrorBase {
  ...
  RoeErrorBase(int32_t c, std::string &&msg)
  ...
  explicit RoeErrorBase(std::string &&msg)
      : code(-1), message(std::move(msg)) {}
};
...
public:
  ...
  // Only enabled when E is derived from RoeErrorBase to avoid ambiguity with
  // value constructor
  template <typename U = E, typename = typename std::enable_if<
                                std::is_base_of<RoeErrorBase, U>::value>::type>
...
template <typename E> class ResultOrError<void, E> {
public:
