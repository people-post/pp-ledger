...
TEST_F(AccountBufferTest, DepositBalance_Success_IncreasesBalance) {
    ...
    auto r = buf.depositBalance(1, 50);
    ASSERT_TRUE(r.isOk());

    auto got = buf.get(1);
    ASSERT_TRUE(got.isOk());
    EXPECT_EQ(got.value().balance, 150);
}
...
TEST_F(AccountBufferTest, DepositBalance_ZeroAmount_Success) {
    ...
    auto r = buf.depositBalance(1, 0);
    ASSERT_TRUE(r.isOk());

    auto got = buf.get(1);
    ASSERT_TRUE(got.isOk());
    EXPECT_EQ(got.value().balance, 100);
}
...
TEST_F(AccountBufferTest, DepositBalance_NegativeAmount_Error) {
    ...
    auto r = buf.depositBalance(1, -10);
...
TEST_F(AccountBufferTest, DepositBalance_AccountNotFound_Error) {
    auto r = buf.depositBalance(999, 100);
    ASSERT_TRUE(r.isError());
...
TEST_F(AccountBufferTest, DepositBalance_Overflow_Error) {
    ...
    auto got = buf.get(1);
    ASSERT_TRUE(got.isOk());
    EXPECT_EQ(got.value().balance, INT64_MAX);
}
...
TEST_F(AccountBufferTest, WithdrawBalance_Success_DecreasesBalance) {
    ...
    auto r = buf.withdrawBalance(1, 30);
    ASSERT_TRUE(r.isOk());

    auto got = buf.get(1);
    ASSERT_TRUE(got.isOk());
    EXPECT_EQ(got.value().balance, 70);
}
...
TEST_F(AccountBufferTest, WithdrawBalance_ZeroAmount_Success) {
    ...
    auto r = buf.withdrawBalance(1, 0);
    ASSERT_TRUE(r.isOk());
...
TEST_F(AccountBufferTest, WithdrawBalance_ExactBalance_Success) {
    ...
    auto r = buf.withdrawBalance(1, 100);
    ASSERT_TRUE(r.isOk());

    auto got = buf.get(1);
    ASSERT_TRUE(got.isOk());
    EXPECT_EQ(got.value().balance, 0);
}
...
TEST_F(AccountBufferTest, WithdrawBalance_NegativeAmount_Error) {
    ...
    auto r = buf.withdrawBalance(1, -5);
    ASSERT_TRUE(r.isError());
    EXPECT_EQ(r.error().code, 11);
...
TEST_F(AccountBufferTest, WithdrawBalance_AccountNotFound_Error) {
    auto r = buf.withdrawBalance(999, 100);
    ASSERT_TRUE(r.isError());
...
TEST_F(AccountBufferTest, WithdrawBalance_InsufficientBalance_Error) {
    ...
    auto got = buf.get(1);
    ASSERT_TRUE(got.isOk());
    EXPECT_EQ(got.value().balance, 50);
}
...
TEST_F(AccountBufferTest, WithdrawBalance_Overflow_Error) {
    auto a = makeAccount(1, INT64_MAX);
    ASSERT_TRUE(buf.add(a).isOk());

    auto r = buf.withdrawBalance(1, 1);
    ASSERT_TRUE(r.isError());
    EXPECT_EQ(r.error().code, 14);
    EXPECT_EQ(r.error().message, "Withdraw would cause balance overflow");
