...
EpochManager::EpochManager(uint64_t slotsPerEpoch, uint64_t slotDuration)
    ...
    log().info << "Epoch manager initialized: " + 
                  std::to_string(slotsPerEpoch_) + " slots per epoch, " +
                  std::to_string(slotDuration_) + "s slot duration";
}

void EpochManager::initializeEpoch(uint64_t epochNumber, const std::string& nonce) {
    EpochInfo info;
...
void EpochManager::finalizeEpoch(uint64_t epochNumber, 
    ...
    log().info << "Finalized epoch " + std::to_string(epochNumber) + 
                  " with " + std::to_string(blockHashes.size()) + " blocks";
}
...
std::string EpochManager::getSlotLeader(uint64_t epochNumber, uint64_t slot) const {
    auto it = epochs_.find(epochNumber);
    if (it == epochs_.end()) {
        return "";
    }
...
int64_t SlotTimer::getSlotEndTime(uint64_t slot, int64_t genesisTime) const {
    return getSlotStartTime(slot, genesisTime) + static_cast<int64_t>(slotDuration_);
}

bool SlotTimer::isTimeInSlot(int64_t timestamp, uint64_t slot, int64_t genesisTime) const {
